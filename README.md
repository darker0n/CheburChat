...
Расширение Chrome (Manifest V3), которое при штатной работе отправляет переписку через ВК в виде шифротекста. Оно защищает содержимое от пассивного хранения и анализа сервером ВКонтакте и СОРМ, а приватный ключ не покидает устройство.
...
Расшифрованный текст рендерится inline в DOM ВК, ввод — в нативном поле. Активный клиентский JS самого ВК технически имеет доступ к DOM/вводу — это **вне** модели защиты беты (граница защиты — пассивный сервер/сеть, не код страницы и не само устройство). Транспорт также может повторять, дублировать и переупорядочивать уже валидные сообщения. Приватный ключ в бете хранится локально без отдельного пароля — известный security debt, закрытие post-beta. Полная модель — в [TECHNICAL.md](docs/TECHNICAL.md).
